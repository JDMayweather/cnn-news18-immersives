...
export default function ScrollScene({
  ...
  return (
    <div className="rm-scene" role="group" aria-label={label}>
      <div className="rm-scene-stage">
        <div className="rm-scene-visual">{children(active)}</div>
        <p className="rm-scene-count" aria-hidden="true">
          {String(active + 1).padStart(2, "0")} / {String(steps.length).padStart(2, "0")}
        </p>
      </div>

      <ol className="rm-scene-steps">
        {steps.map((step, i) => (
          <li
            key={step.k}
            ref={(el) => {
            ...
            className={`rm-step${i === active ? " is-on" : i < active ? " is-past" : ""}`}
          >
            <span className="rm-step-rail" aria-hidden="true">
              <i />
            </span>
            <span className="rm-step-k">{step.k}</span>
            <span className="rm-step-v">{step.v}</span>
          </li>
        ))}
      </ol>
    </div>
