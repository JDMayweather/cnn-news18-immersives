...
/**
 * Soft reveals. Every `.nv-rise` in the story fades up once, on entry, with a
...
export function useRiseObserver(): void {
  useEffect(() => {
    ...
    const elements = Array.from(document.querySelectorAll<HTMLElement>(".nv-rise:not(.is-in)"));
